...
export default function BucketListCard({ item, user, onEdit, onDelete }: BucketListCardProps) {
  const badgeColor = item.completed
  ...
  item.completed
    ? 'bg-muted border border-muted text-muted-foreground line-through shadow-inner'
    : item.isFavorite
    ...
    >
      <div className="flex-1">
        <Link href={`/bucket/${item.id}`} className="group inline-flex items-center">
          <h3 className="text-lg font-semibold capitalize text-foreground cursor-pointer transition relative inline-flex items-center gap-[5px] hover:underline underline-offset-4">
            {item.name}
